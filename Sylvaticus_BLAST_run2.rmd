...
unique_consensus_ID[[1]] %>% group_by(species) %>% arrange(desc(bitscore)) %>% filter(species = "Rattus")
...
# trying to save the alignment
```{r}
print(Rodent_species_alignment, show="complete") # shows full alignemnt

# this saves it but it has missing sequences
sink("Rodent_species_alignment.txt")
print(Rodent_species_alignment, show="complete")
sink()
```
...
# this method is shown in the docummentation but for some reason it doesnt work for me
```{r}
 msaPrettyPrint(Rodent_species_alignment, output="pdf", showNames="none",
               showLogo="none", askForOverwrite=FALSE, verbose=FALSE)
# need to install package tinytex::install_tinytex()
...
msaPrettyPrint(Rodent_species_alignment, file = NULL)



texi2dvi('Rodent_species_alignment.tx',clean=TRUE)
```
